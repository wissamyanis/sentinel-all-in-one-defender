...
> **Fully automatic option:** the Analytics Rules tab has an **"Automatically enable ALL matching rule templates after deployment"** checkbox (default off). When checked, the deployment creates a small **Azure Automation Account** with a system-assigned managed identity (granted Microsoft Sentinel Contributor), imports a runbook (`Scripts/EnableRulesRunbook.ps1`), and starts it once — so all matching rules are enabled automatically with no manual step. This uses **Azure Automation's managed sandbox, not `deploymentScripts`**, and authenticates via the managed-identity endpoint, so it **does not need storage account keys** and works on tenants that block them (unlike the original v2 approach). The rules appear a **few minutes after** the deployment completes, because the runbook runs asynchronously. Requires the tenant to allow the **`Microsoft.Automation`** resource provider. If you prefer to keep it manual, leave it unchecked and use the Outputs command below.
